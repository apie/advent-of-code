...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                stack[dest] := stack[src][1] + stack[dest];
                stack[src] := copy(stack[src], 2, length(stack[src]));
            END;
        END;
    END;
    close(f);
...
VAR
    a: string;
BEGIN{d5a}
    assert(answer(testfile) = 'CMZ', 'test faal');
    a := answer(filename);
    assert(a <> 'CMZWCZTHTMPS');
    assert(a <> 'WCZTHTMPSWCZTHTMPS');
    writeln('');
