...
BEGIN
    ...
    BEGIN
        ...
        {If one range partially contains the other, add it to our count}
        {left pair contains right pair}
        IF (range[1] <= range[3]) AND (range[2] >= range[3]) THEN answer += 1
        {right pair contains left pair}
        ELSE IF (range[3] <= range[1]) AND (range[4] >= range[1]) THEN answer += 1;
    END;
    close(f);
END;
...
VAR
    a : integer;
BEGIN{d4b}
    assert(answer(testfile) = 4, 'test faal');
    a := answer(filename);
    assert(a > 730, 'too low');
    writeln('');
    writeln('answer: ', a);
END.
