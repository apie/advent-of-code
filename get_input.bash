...
if [ ! -f "$DIR"/cookie.txt ]
then
  echo 'No cookie found. Format of cookiefile: session=abcde'
  exit 1
fi
COOKIE=$(cat "$DIR"/cookie.txt)
YEAR=$1
DAY=$(($2)) # Convert to int to strip leading zero
FILENAME="$DIR"/$YEAR/d"$(printf "%02d.input" "$DAY")"
curl "https://adventofcode.com/$YEAR/day/$DAY/input" -H "Cookie: $COOKIE" > $FILENAME
