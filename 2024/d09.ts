import "./util.ts";
import { isRunningInTest, p } from "./util.ts";
...
const checksum = (filemap: Map<number, ifilesystem>): number =>
    Array.from(
        filemap.values().filter((curobj) => curobj.type === "file").map(
            (curobj) => {
                let tot = 0;
                for (let plen = 0; plen < curobj.len; plen++) {
                    tot += curobj.id! * (curobj.pos + plen);
                }
                return tot;
            },
        ),
...
const compactWholeFiles = (filemap: Map<number, ifilesystem>): void => {
    p("compacting..");
    Array.from(filemap.values().filter((curobj) => curobj.type === "file"))
        .sort((a, b) => -1 * (a?.id || 0) + (b?.id || 0)).forEach((lastobj) => {
            //find free space
            const freeobj = Array.from(
                filemap.values().filter((curobj) => curobj.type === "free"),
            ).sort((a, b) => a.pos - b.pos).find((curobj) =>
                curobj.len >= lastobj.len
            );
            if (!freeobj) return;
